...
public abstract class LeapArray<T> {
    ...
    protected int windowLengthInMs;      // 每个时间窗格的时间间隔
    protected int sampleCount;                // 一个时间窗口有多少个时间窗格
    protected int intervalInMs;                 // 一个时间窗口的时间间隔
    ...
    public WindowWrap<T> currentWindow(long timeMillis) {
        ...
        while (true) {
            ...
            if (old == null) {
                ...
                /*
                 *  timestamp：2300  ---> idx: (2300/200)%5=1  ===> old，窗格索引位置是1代表第二个窗格（B1）
                 *                                 ---> windowstart: 2200       ===> 当前时间戳所在窗格的起始时间
                 *                      - 此时 windowStart > old ---> 需要去重置这个 old bucket，更改起始时间为2200，value重置
                 *                                                               ---> 发现没，这5个窗格bucket建好后，不会再去创建bucket，只需要修改它的起始时间就能实现
                 *                                                                       滑动效果，这样就不需要去不断重复的去创建对象了
                 *                      - 还有个相当重要的一点（非常重要），重置这个bucket之后，逻辑上讲这个窗格跑到了最后一个位置，
                 *                         但是，实际上它的物理位置（也就是这个bucket在数组中的索引位置）是不变的
                 *                      - 上一点很重要，怪不得我计算idx后感觉对不上位置，修改的只是每个bucket的属性值，索引位置是不动的呀
                 *                      - circle array
                 *
                 *    (old)
                 *             B0       B1      B2    NULL      B4
                ...
                if (updateLock.tryLock()) {
                    try {
                        ...
                        // 更新old bucket 的开始时间为 windowStart，然后重置old bucket的value值 ---> 此时 old bucket 就变成了起始时间为2200的那个bucket了
                ...
                // 时钟回拨 -> 就是服务器时间往前调了 ---> 会去新建一个单独的时间窗格
                // 正常是不会也不应该走这里的，应该是为了这个框架的完整性
